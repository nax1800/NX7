...
namespace Stats
{
	void ToggleDirectorsCut()
	{
		if (Defines::Stats::IsDirectorsCut())
			Utils::ChangeValue<int>(Offsets::Stats::DirectorsCut, 0);
		else
			Utils::ChangeValue<int>(Offsets::Stats::DirectorsCut, Defines::Game::DirectorsCut);
	
		LogNX("Unlocked Director's Cut: {}", Defines::Stats::IsDirectorsCut() ? "True" : "False");
	}
